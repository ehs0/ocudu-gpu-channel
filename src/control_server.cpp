  ...
  while (!stop_requested_.load(std::memory_order_relaxed)) {
    ...
    for (const auto& [link_id, ctl_ptr] : link_map_) {
      ...
      f << link_id << " "
        ...
        << "\"warmup_begin_slot\":" << ts.warmup_begin_slot << ","
        << "\"warmup_end_slot\":" << ts.warmup_end_slot << ","
        << "\"warmup_begin_unix_ns\":" << ts.warmup_begin_unix_ns << ","
        << "\"warmup_end_unix_ns\":" << ts.warmup_end_unix_ns << ","
        << "\"slot_processing\":{"
